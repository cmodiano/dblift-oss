...
def test_builtin_extension_commands_are_registered_on_core_parser():
    parser = create_parser()
    subparser_actions = [
        action for action in parser._actions if isinstance(action, argparse._SubParsersAction)
    ]
    assert len(subparser_actions) == 1
    commands = subparser_actions[0].choices
...
def test_builtin_extension_handlers_are_available_to_cli_dispatch():
    for command in [
        "diff",
        "validate-sql",
        "plan",
        ...
        assert command in _AVAILABLE_COMMANDS


def test_pyproject_registers_builtin_command_entry_points():
