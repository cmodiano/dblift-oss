...
class TestSqlServerHistoryManager(unittest.TestCase):
    ...
    def test_to_python_string_with_none(self):
        from db.plugins.sqlserver.sqlserver.history_manager import to_python_string

        self.assertIsNone(to_python_string(None))

    def test_to_python_string_with_java_object(self):
        from db.plugins.sqlserver.sqlserver.history_manager import to_python_string

        obj = MagicMock()
        obj.toString.return_value = "java_string"
        self.assertEqual("java_string", to_python_string(obj))

    def test_to_python_string_with_regular_object(self):
        from db.plugins.sqlserver.sqlserver.history_manager import to_python_string

        self.assertEqual("42", to_python_string(42))
