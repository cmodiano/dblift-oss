...
class TestNoLocalDialectMap:
    """AC#2.2 + AC#2.3 — sqlglot_parser and sql_insights no longer have local DIALECT_MAP."""
    ...
    def test_sql_insights_no_local_dialect_map(self):
        """Module no longer defines its own DIALECT_MAP at module level."""
        import core.migration.sql.sql_insights as mod
    ...
    def test_sql_insights_uses_quirks_for_dialect(self):
        from core.migration.sql import sql_insights as mod

        module_source = inspect.getsource(mod)
        # SqlInsights now resolves sqlglot dialect via quirks, not a local map.
        assert "quirks.sqlglot_dialect" in module_source
        assert "ProviderRegistry" in module_source
