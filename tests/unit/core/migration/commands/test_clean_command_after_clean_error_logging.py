...
class TestCleanCommandAfterCleanErrorLogging:
    ...
    def _make_command(self):
        ...
        provider.clean_schema.side_effect = RuntimeError("clean failed")
        return cmd, provider, log

    @staticmethod
    def _after_clean_error_raises(*args, **kwargs):
        """Side effect that only raises for afterCleanError callbacks, so clean_schema is reached."""
        if args[1] == "afterCleanError":
            raise RuntimeError("callback failed")

    def test_after_clean_error_callback_exception_logs_debug(self):
        """AC#2.1: clean_schema raises + _execute_callbacks(afterCleanError) raises → log.debug called."""
