...
@pytest.mark.unit
def test_pyproject_distribution_name_is_dblift():
    with (ROOT / "pyproject.toml").open("rb") as handle:
        pyproject = tomllib.load(handle)

    assert pyproject["project"]["name"] == "dblift"
...
def test_package_version_matches_project_metadata():
    ...
    assert f'__version__ = "{pyproject["project"]["version"]}"' in init_text
...
@pytest.mark.unit
def test_all_extra_self_references_dblift_distribution_name():
    with (ROOT / "pyproject.toml").open("rb") as handle:
        pyproject = tomllib.load(handle)

    assert pyproject["project"]["optional-dependencies"]["all"] == [
        "dblift[postgresql,mysql,mariadb,oracle,sqlserver,db2]"
    ]
