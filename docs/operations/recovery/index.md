...
## Transactionality by dialect

Oracle and MySQL autocommit DDL statement-by-statement, so a multi-statement
migration that fails partway through can leave partial state behind — this is
why the [partial DDL runbook](partial-ddl-mysql.md) exists. PostgreSQL and SQL
Server run DDL inside a transaction and roll back cleanly on failure.

When `dblift migrate` completes successfully, the schema matches every
migration file up to and including the highest applied version, and the
journal (`dblift_schema_history`) reflects exactly what was executed.
