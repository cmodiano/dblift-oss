...
4. **provider.py**, **quirks.py** — implement behaviour and any required hooks. Start with the generated stubs. For pure SQLAlchemy drivers you usually only need to subclass `SqlAlchemyProvider`. See `db/base_quirks.py` for the full recipe (generators, parsers, introspection, the five manager components, etc.).
